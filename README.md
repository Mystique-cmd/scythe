...
| **Auth Bypass via State Manipulation** | Detects 10 categories of authentication bypass through state/token manipulation: JWT Manipulation (alg:none, algorithm confusion, JWK injection, kid injection, empty/weak signature), Session Fixation (token in URL, weak/predictable session IDs, weak bearer tokens), OAuth State Parameter Weakness (missing/static/predictable state, CSRF), Password Reset Token Weakness (numeric/timestamp/base64 tokens, user enumeration), Session Attribute Manipulation (client-controlled `isLoggedIn`, `authenticated`, impersonation params), MFA/2FA Bypass (`mfa_verified`, `skip_mfa` params, weak OTP codes, JWT MFA claims), Remember-Me Weakness (base64 credentials, weak persistent cookies, excessive cookie lifetime), Authorization Header/Token Injection (multiple auth mechanisms, token in query string, weak API keys), Email/SMS Verification Bypass (`email_verified`, `skip_verification`, GraphQL verification mutations), and Role/Scope Manipulation in Token Claims (admin roles, broad scopes, client-controlled role params, suspicious JWT issuer). | JWT alg:none/confusion/kid injection -> `high`; MFA/2FA bypass -> `high`; Verification bypass -> `high`; Session attribute/role manipulation -> `high`; Password reset weakness (numeric/timestamp) -> `high`; Remember-me with credentials -> `high`; OAuth missing/static state -> `high`; All other patterns -> `high`/`medium` |
| **Prototype Pollution** | Detects 12 categories of prototype pollution vulnerabilities in JavaScript/Node.js applications: `__proto__` Key Injection (direct `__proto__` keys in JSON body, query params, GraphQL variables), constructor.prototype Manipulation (nested prototype chains bypassing shallow filters), Merge/Assign Function Abuse (payloads targeting merge utilities like lodash.merge, jQuery.extend), URL-Encoded Nested Param Pollution (bracket notation `__proto__[key]`), JSONPath / Deep Set Expression Injection (`$.constructor.prototype`, lodash.set paths), Array-Based Prototype Pollution (pollution via array elements and numeric keys), HTTP Header-Based Pollution (custom override headers merged into objects), Cross-Request Merge Chain Detection (multi-step sequential merge pollution), Cookie/Body-Based Prototype Pollution (`__proto__` in cookies and form bodies), Sensitive Property Override Detection (overriding `toString`, `valueOf`, `hasOwnProperty`, `constructor`), Nested Merge Loop Detection (deeply nested objects with repeated `__proto__` designed for recursive merge exploits), and Response-Based Prototype Pollution Confirmation (server reflection and error messages confirming successful pollution). | Direct `__proto__` injection -> `high`; constructor.prototype manipulation -> `high`; Merge endpoint abuse -> `high`; URL-encoded bracket notation -> `high`; JSONPath/deep-set expressions -> `high`; Array-based pollution -> `high`; Header override with proto -> `high`; Cross-request merge chain -> `high`; Cookie/body proto injection -> `high`; Sensitive property override -> `high`; Deep nested merge loop -> `high`/`medium`; Response confirmation -> `high`/`medium` |
| **Memory / Web Context Issues** | Detects 10 categories of memory-related vulnerabilities in web browser contexts: DOM Clobbering (HTML `id`/`name` collisions with dangerous properties like `innerHTML`, `cookie`, `location`), Cache Poisoning & Web Cache Deception (aggressive/misconfigured cache headers, cache key manipulation, static extension on auth content), Client-Side Storage Manipulation (payloads targeting `localStorage`/`sessionStorage`, storage injection, quota abuse), Memory Exhaustion via Deeply Nested Objects (JSON bombs, deep nesting >12 levels, large arrays >5K elements, compression bombs), Service Worker Cache Pollution (SW script caching abuse, cross-origin imports, Cache API misuse, stale cache serving), Detached DOM / Large Payload Generation (massive HTML >10MB, excessive DOM operations, cloned nodes, DOM bloat), Cross-Window/Tab Memory Leak (`window.open` without cleanup, `postMessage` listener accumulation, BroadcastChannel abuse), WebSocket Connection Abuse (repeated upgrades, unclosed connections, message flooding, rapid reconnections), IndexedDB/Storage API Abuse (excessive database creation, unbounded queries, orphaned transactions), and Event Listener/Timer Leak Indicators (`setInterval` without `clearInterval`, orphaned `addEventListener`, undisconnected Observers, uncancelled animation frames). | DOM clobbering (innerHTML/cookie/location) -> `high`; Cache poisoning on auth content -> `high`; Storage injection payload -> `high`; JSON bomb / compression bomb -> `high`; SW cross-origin import -> `high`; DOM bloat >5000 rows -> `high`; postMessage without origin check -> `high`; WebSocket flood/unclosed -> `high`; Orphaned intervals/listeners -> `high`/`medium`; Excessive max-age -> `medium`; Storage quota abuse -> `medium`; Observer leaks -> `medium`; Deep nesting -> `medium`; All other patterns -> `medium`/`low` |
...
└── src/
    ...
    └── ui/
        ...
        ├── flaws.js              # Logic flaws detection engine (7 detectors + file parser vulns + auth bypass state manip)
        ...
        ├── auth-bypass-state-manip.js  # Auth bypass via state manipulation detection (10 detectors)
        ├── memory-web-context.js  # Memory-related issues in web context detection (10 detectors)
